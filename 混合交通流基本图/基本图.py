...
gamma0 = 0.7
gamma1 = 0.3
list1 = np.arange(1,200,4)
list2 = np.arange(200,300,20)
list_all = np.hstack((list1,list2))
for veh_num in list_all:
    veh_ar = np.zeros([veh_num, list_num])
    veh_ar[:, 0] = np.arange(0, veh_num)
    veh_ar[:, 2] = lane_length * veh_ar[:, 0] / veh_num
    veh_ar[:,5] = np.random.choice([0,1],veh_num,p=[0.5,0.5])
    veh_total = []
    for t in range(sim_time):
        headway = headway_identify(veh_ar)
        delta_v = delta_v_identift(veh_ar)
        acc = IDM(veh_ar[:, 3], headway, delta_v, 2, 1.5, 30, 2, 1)
    ...
    density = veh_num/lane_length
    speed_mean = veh_total[:,3].mean()
    quantity = density*speed_mean*3600
    quantity_list.append(quantity)
    density_list.append(density)
    # plt.scatter(veh_total[:,2],veh_total[:,1],s=0.3)
    # plt.show()
    print(veh_num)
plt.plot(density_list,quantity_list)
plt.show()