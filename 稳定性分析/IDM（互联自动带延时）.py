...
def Acc_upgrade():
    ar[:, 6] = ar[:, 4]
    ACC = ar[ar[:, 0] < N - 2]
    ACC[:, 4] = MAC_IDM(ACC[:, 11], gamma1 * ACC[:, 12] + gamma2 * ACC[:, 14], gamma1 * ACC[:, 13] + gamma2 * ACC[:, 15], T)
    ar[ar[:, 0] < N - 2] = ACC
    ar[N - 1][4] = 0
    ar[N - 2][4] = IDM(ar[N - 2][11], ar[N - 2][12], ar[N - 2][13], T)
...
for alpha in np.linspace(0.1,4,20):
    for T in np.linspace(0.1,4,20):
        ...
        for i in np.arange(0, 2000):
            if i>5:
                ar[:, 11] = arr_list[arr_list[:, 1] == ar[0, 1] - tao][:, 3]
                ar[:, 12] = arr_list[arr_list[:, 1] == ar[0, 1] - tao][:, 7]
                ar[:, 13] = arr_list[arr_list[:, 1] == ar[0, 1] - tao][:, 8]
                ar[:, 14] = arr_list[arr_list[:, 1] == ar[0, 1] - tao][:, 9]
                ar[:, 15] = arr_list[arr_list[:, 1] == ar[0, 1] - tao][:, 10]
            ar = Spacing_identify(ar)
            ...
            ar[:, 1] += 1
            list.append(ar)
            arr_list = np.array(list).reshape(-1, 16)
